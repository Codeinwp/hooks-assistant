(function ($) {
	...
	$('.cmb-repeatable-group').on('cmb2_add_group_row_start', function (event, instance) {
		...
		$oldRow.find('.hk_select2').each(function () {
	...
	$('.cmb-repeatable-group').on('cmb2_add_row', function (event, newRow) {
		$(newRow).find('.hk_select').each(function () {
		...
		$(newRow).find('.hk_multiselect').each(function () {
			$('option:selected', this).removeAttr("selected");
			$(this).select2_sortable();
		});

		// Reinitialise the field we previously destroyed
		$(newRow).prev().find('.hk_select').each(function () {
			$(this).select2({
				allowClear: true
			});
		});
		...
		// Reinitialise the field we previously destroyed
		$(newRow).prev().find('.hk_multiselect').each(function () {
			$(this).select2_sortable();
		});
	});
	...
	$('.cmb-repeatable-group').on('cmb2_shift_rows_start', function (event, instance) {
		var grouhkrap = $(instance).closest('.cmb-repeatable-group');
		grouhkrap.find('.hk_select2').each(function () {
			$(this).select2('destroy');
		});
	...
	$('.cmb-repeatable-group').on('cmb2_shift_rows_complete', function (event, instance) {
		var grouhkrap = $(instance).closest('.cmb-repeatable-group');
		grouhkrap.find('.hk_select').each(function () {
			$(this).select2({
				allowClear: true
			});
		});

		grouhkrap.find('.hk_multiselect').each(function () {
			$(this).select2_sortable();
		});
	});
